    ...
    def __init__(self, SCREEN_WIDTH, SCREEN_HEIGHT, BLOCK_WIDTH, SIZE, BORDER_WIDTH):
        ...
        while True:
            ...
            if not self.myhouse.game_over:
                ...
                if cur_drop_time - self.myhouse.myTimer.last_drop_time > self.myhouse.myTimer.interval:
                    if not self.myhouse.pause:
                        # 不应该在下落的时候来判断到底没，我们玩俄罗斯方块的时候，方块落到底的瞬间是可以进行左右移动
                        if not self.myhouse.judge(self.myhouse.cur_pos_x,
                                                  self.myhouse.cur_pos_y + 1, self.myhouse.cur_block):
                            self.myhouse.dock()
                        else:
                            self.myhouse.myTimer.last_drop_time = cur_drop_time
